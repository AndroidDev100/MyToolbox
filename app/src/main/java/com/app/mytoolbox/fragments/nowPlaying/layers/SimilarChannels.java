package com.app.mytoolbox.fragments.nowPlaying.layers;
...
import com.app.mytoolbox.beanModel.ksBeanmodel.AssetCommonUrls;
...
public class SimilarChannels {
    ...
    private static SimilarChannels movieDescriptionRepository;
    public static SimilarChannels getInstance() {
        if (movieDescriptionRepository == null) {
            movieDescriptionRepository = new SimilarChannels();
        }
    ...
    public LiveData<List<AssetCommonBean>> loadData(final Asset asset, Context ctx) {
        ...
        ksServices.getSimilarChannel(1,assetType, liveGenre, (status, result) -> {
    ...
    private void parseLiveNowAssests(Response<ListResponse<Asset>> assetList) {
        ...
        setRailData(responseList, assetCommonBean, AppLevelConstants.TYPE4);
