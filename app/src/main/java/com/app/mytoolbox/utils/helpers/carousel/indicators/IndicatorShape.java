...
public abstract class IndicatorShape extends androidx.appcompat.widget.AppCompatImageView {
    ...
    public IndicatorShape(Context context) {
        super(context);
    }
    ...
    private void setup() {
        if (this.indicatorSize == 0) {
            indicatorSize = getResources().getDimensionPixelSize(R.dimen.default_indicator_size);
        }
        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(indicatorSize, indicatorSize);
        int margin = (int) getResources().getDimension(R.dimen.default_indicator_margins);
